...
function create() {
    ...
    for (var y = 0; y < h; y++)
    {
        for (var x = 0; x < w; x++)
        {
            ...
            var from = Phaser.BitmapData.getTransform(game.world.centerX, game.world.centerY, 1, 0);
            var to = Phaser.BitmapData.getTransform(x * tileWidth, y * tileHeight, 1, 1);

            data.push(game.make.tween(from).to(to, 1000, Phaser.Easing.Linear.None).generateData(60));

            times.push( { start: game.time.now + delay, i: 0, started: false, finished: false, step: game.rnd.between(1, 1) });
            // times.push( { start: game.time.now, i: 0, started: false, finished: false, step: game.rnd.between(1, 1) });
