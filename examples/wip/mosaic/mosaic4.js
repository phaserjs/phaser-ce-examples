...
function create() {
    ...
    var from = Phaser.BitmapData.getTransform(0, 0, 0, 0);
    var to = Phaser.BitmapData.getTransform(0, 0, 1, 1);
    data = game.make.tween(from).to(to, 1000, Phaser.Easing.Linear.None).generateData(60);
    ...
    if (running)
    {
        ...
        for (var y = 0; y < h; y++)
        {
            for (var x = 0; x < w; x++)
            {
                ...
                bmdDest.blit(bmdSourceB, r1, r2, 1, data[i]);
